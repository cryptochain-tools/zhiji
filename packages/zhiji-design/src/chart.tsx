...
/**
 * Chart v2.2 redesign:
 *  - Tooltip: `border-border-subtle bg-popover shadow-card` (unified with
 *    Select / Dropdown / Popover popups, no more border-border/50).
 *  - Recharts axis text: `text-foreground-tertiary` (was muted-foreground).
 *  - Grid lines: `stroke-border-subtle/60` (lighter).
 *  - Tooltip cursor (hover bg): `fill-muted` retained.
 *  - Legend icon size: 10 px → 8 px squares (tighter, modern look).
 *
 *  Note: chart.tsx uses Recharts' built-in color string passing for series
 *  fills — those rely on `--color-{key}` CSS vars defined by ChartStyle,
 *  which is the documented exception to the hex-literal rule.
 */
...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-lg",
        "border border-border-subtle bg-popover px-3 py-2 text-xs",
        "shadow-card",
        className
      )}
    >
