...
/**
 * Select v2.2 redesign:
 *  - Trigger default height aligned to h-9 (36 px) — matches Input v2.2
 *    so forms no longer have mixed-height controls.
 *  - Trigger focus glow (ring-3 ring-ring/50) matches Input and Button.
 *  - Popup gains `border border-border-subtle` + `shadow-card` instead of
 *    the heavier `ring-1 ring-foreground/10` — lighter, more refined.
 *  - SelectItem hover uses `bg-accent text-accent-foreground` (brand-tinted).
 */
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "bg-card py-2 pr-2 pl-2.5 text-sm whitespace-nowrap select-none",
        // Height — aligned to Input
        "data-[size=default]:h-9 data-[size=sm]:h-7 data-[size=sm]:rounded-md",
        // Transitions
        "transition-[border-color,background-color,box-shadow] duration-fast ease-out outline-none",
        // Focus glow — matches Input v2.4
        ...
        // Dark — bg-card already handles light/dark; only invalid ring needs dark variant
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            ...
            // Lighter border + card shadow (vs old ring-foreground/10)
            "border border-border-subtle bg-popover text-popover-foreground shadow-card",
            "duration-100",
