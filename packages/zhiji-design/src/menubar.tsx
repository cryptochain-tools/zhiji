...
/**
 * Menubar v2.1 redesign — 与 Dropdown / Dialog token 体系对齐：
 *  - Shell: 显式 `bg-card border-border-subtle`，加微妙底部阴影做层次。
 *  - Trigger: `rounded-sm` → `rounded-md`（与弹层条目同级），加 `data-open` 高亮。
 *  - Content / SubContent: `ring-foreground/10` → `ring-border-subtle`；
 *    `shadow-md` / `shadow-lg` → `shadow-card`（统一阴影 token）。
 *  - Separator: `bg-border` → `bg-border-subtle`（更克制的分隔线，与 Dropdown 一致）。
 *  - 内部条目（Item / CheckboxItem / RadioItem / SubTrigger / Label / Shortcut）
 *    完全代理 DropdownMenu，弹层视觉自动跟随 Dropdown 的改动。
 */
...
function Menubar({ className, ...props }: MenubarPrimitive.Props) {
  return (
    <MenubarPrimitive
      data-slot="menubar"
      className={cn(
        "inline-flex h-8 items-center gap-0.5 rounded-lg border border-border-subtle bg-card p-[3px] shadow-[0_1px_0_var(--border-subtle)]",
        className,
      )}
      {...props}
    />
...
function MenubarTrigger({
  ...
  return (
    <DropdownMenuTrigger
      ...
      className={cn(
        "flex h-6 items-center rounded-md px-2 py-1 text-sm font-medium text-foreground",
...
function MenubarContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn(
        "min-w-36 rounded-lg bg-popover p-1 text-popover-foreground",
...
function MenubarSubContent({
  ...
  return (
    <DropdownMenuSubContent
      ...
      className={cn(
        "min-w-32 rounded-lg bg-popover p-1 text-popover-foreground",
        "shadow-card ring-1 ring-border-subtle",
        "duration-fast ease-out",
        "data-[side=bottom]:slide-in-from-top-2 data-[side=top]:slide-in-from-bottom-2",
        "data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2",
